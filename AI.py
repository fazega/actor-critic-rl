...
class AI:
    def __init__(self):
        ...
        self.reflex = Reflex()
        self.epsilon_random = lambda t : 0.7*(1/(1+0.0005*t)) #proba de tirer une action random pour l'exploration
    ...
    def updateNetworks(self):
        while True:
            ...
            while len(self.data_in) < 2*self.depth:
                self.thread_signal.wait()

            for k in range(len(self.data_in)-self.depth):
                trajectory = self.data_in[k:k+self.depth]
